...
class BaseStrategy:
    ...
    Каждая новая стратегия наследуется и переопределяет:
      - _make_features(df, **params) -> pd.DataFrame с признаками
    ...
    def _make_features(self, df, **params):
        """Создать признаки для модели. Возвращает DataFrame."""
        return df

    def calc_indicator(self, df, **params):
        """Рассчитать индикатор/сигналы. Возвращает DataFrame с колонками сигнала."""
        return df
    ...
    def backtest(df, sl_points, tp_points, point, tick_value, tick_size,
        ...
        # Найти колонки сигнала — ищем 'signal' в имени
        signal_col = None
        for col in ['signal', 'lr_signal', 'rf_signal', 'stoch_signal', 'ma_signal']:
